...
unsigned char Check, T_byte1, T_byte2,
 ...
 char water2[10];
void StartSignal(){
     TRISD.B0 = 0;  //RD0'ý çýkýþ olarak yapýlandýrýldý
     PORTD.B0 = 0;  //RD0 sensöre 0 gönderir
     delay_ms(18);
     PORTD.B0 = 1;   //RD0 sensöre 1 gönderir
     delay_us(30);
     TRISD.B0 = 1;   //RD0'ý giriþ olarak yapýlandýrýn
 }
 void CheckResponse(){        //sinyal geldiði sürece sürekli çalýþmasýný saðlýyor
      Check = 0;
      delay_us(40);
      if (PORTD.B0 == 0){
         delay_us(80);
 ...
 char ReadData(){
      char i, j;
      for(j = 0; j < 8; j++){
            while(!PORTD.B0);  //PORTD.B0 YÜKSEK olana kadar bekleyin
               delay_us(30);
               if(PORTD.B0 == 0)
...
void main() {
     ...
     Lcd_Init();
     while(1){
        StartSignal(); //FONKSÝYON
        CheckResponse(); //FONKSÝYON
        if(Check == 1){   //KONTROL
           RH_byte1 = ReadData();
           RH_byte2 = ReadData();
           T_byte1 = ReadData();
           ...
           if(Sum == ((RH_byte1+RH_byte2+T_byte1+T_byte2) & 0XFF)){ 
              Temp= T_byte1;
              RH = RH_byte1;      //SICAKLIK VE NEM SENSÖRLERÝ
              water = ADC_Read(3)/10;     //su seviyesi sensörünün adc iþlemi
              basinc = ADC_Read(0)*10 / 8.39215686;   //basýnc sensörünün adc iþlemi
              ...
              Lcd_Cmd(_LCD_CURSOR_OFF);
              Lcd_Cmd(_LCD_CLEAR);
              if((Temp<=5)&(RH<=20)&(basinc>1020)){  //koþullarýn   yazýlmasý
              ...
              if((Temp<=15)&(RH>45)&(basinc<1020)){  //koþullarýn   yazýlmasý
              ...
              if((Temp>30)&(RH>=75)&(basinc<1000)){  //koþullarýn   yazýlmasý
